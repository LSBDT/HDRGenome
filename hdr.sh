...
perl bin/moirai2.pl -s 1 -m 1 -i 'root->project->$project' -o '$project->flag/dircreated->T' command << 'EOF'
mkdir -p $project
mkdir -p $project/log
EOF

echo "# List input directory..."
perl bin/moirai2.pl -s 1 -x -m 1 -i '$project->indir->$input' -o '$project->case->$basename,$basename->input->$path' ls

echo "# List position directory..."
perl bin/moirai2.pl -s 1 -x -m 1 -i '$project->positiondir->$input' -o '$basename->$project/position->$path' ls

echo "# Creating VCF table..."
perl bin/moirai2.pl -s 60 -m 1 -i 'root->project->$project,$project->indir->$directory,$project->qvThreshold->$qvThreshold' -o '$project->vcftable->$output,$project->vcfTableLog->$logFile' command 'output=$project/vcftable.txt' << 'EOF'
...
#AR/AD/DD
perl bin/moirai2.pl -s 10 -m 1 -i '$project->case->$case,$case->$project/hdr->$hdr' -o '$case->$project/stats->$output,$case->$project/statsLog->$logFile' -O "Results in file" command << 'EOF'
